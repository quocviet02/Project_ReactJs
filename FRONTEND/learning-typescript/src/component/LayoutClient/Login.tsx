  ...
  const onSubmit: SubmitHandler<IUser> = async (user) => {
    try {
      const  {data}  = await axios.post(
        "http://localhost:8000/blogs/signin",user
      );
      console.log(data);
          ...
          <div className="formSignIn text-center mt-[40px]">
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {errors.email && errors.email.type === "required" &&(
                  <span className=" text-gray-700 text-[14px]  mr-[230px]">*This field is required.</span>
                )}
              <br />
            ...
            <label className="mr-[290px]  font-bold text-[#4A5568]">
              ...
              {errors.password && errors.password.type ==="minLength"&&(
                <span className=" text-gray-700 text-[14px]  mr-[70px]">*Your password must have at least 8 characters.</span>
              )}
            </div>
            ...
            >
              <p className="my-[20px]">Forgot password?</p>
            </Link>
